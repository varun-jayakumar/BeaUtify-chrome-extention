...
function replaceAllButtons() {
  ...
  for (let i = 0; i < 16; i++) {
    if (i % 2 != 0) {
      // linkList[i].setAttribute("style", "margin-bottom: 10px");
      //et bgColor = linkList[i].parentElement.bgColor;
    ...
    linkList[0].setAttribute(
      "style",
      `background-color:##FF0000; margin-top:10px; margin-left:0.7em; margin-bottom: 0.4em;`
    );
    linkList[2].setAttribute(
      "style",
      `background-color:#FF0000; margin-top:10px; margin-bottom: 0.4em;`
    );
    linkList[4].setAttribute(
      "style",
      `background-color:#FF0000; margin-top:10px; margin-bottom: 0.4em;`
    );
    linkList[6].setAttribute(
      "style",
      `background-color:#FF0000; margin-top:10px; margin-bottom: 0.4em;`
    );
    linkList[8].setAttribute(
      "style",
      `background-color:#FF0000; margin-top:10px; margin-bottom: 0.4em;`
    );
    linkList[10].setAttribute(
      "style",
      `background-color:#FF0000;margin-top:10px; margin-left:0.7em; margin-bottom: 0.4em;`
    );
    linkList[12].setAttribute(
      "style",
      `background-color:#FF0000; margin-top:10px; margin-bottom: 0.4em;`
    );
    linkList[14].setAttribute(
      "style",
      `background-color:#FF0000; margin-top:10px; margin-bottom: 0.4em;`
    );
  }
...
function changeWidthOfTable() {
  const tables = document.querySelectorAll("table");
  console.log(tables);
  tables[4].setAttribute("width", "500");
  tables[5].setAttribute("width", "500");
}

function makeLinkCellWide() {
  const allTd = document.querySelectorAll("table>tbody>tr>td");
  allTd[4].setAttribute(
    "style",
    "width:1500px; display: flex; flex-direction: row;  text-decoration: none; margin: 1.5em; justify-content:center"
  );
  const allATag = document.querySelectorAll("table>tbody>tr>td>a");
  for (let i = 0; i < 4; i++) {
    // allATag[i].setAttribute("style", "text-decoration: none;");
  }
